...
import com.lifesolutions.bd.kotlinCode.data.model.CampusAmbassadorModel
...
class CampussCampusAmbassadorModelViewAdapter(
    
    private val context: Context,
    private val users: List<CampusAmbassadorModel>
) : RecyclerView.Adapter<CampussCampusAmbassadorModelViewAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.designation.text = user.bio
...
//        getOnlineStatus(user._id!!, holder.activeBadge)
    ...
    private fun showDialog(ambassador: CampusAmbassadorModel) {
        ...
        // Set Value
        if (ambassador.image != null && ambassador.image != "none") {
            Glide.with(context)
                .load(ambassador.image)
                .into(image)
        } else {
            Glide.with(context)
                .load(R.drawable.user_low)
        ...
        name.text =  "${ambassador.name} "
        designation.text = ambassador.bio
        ...
        btnCall.setOnClickListener {
            callUserAudio(ambassador.userId!!, epicDialog)
        }
        btnChat.setOnClickListener {
            Intent(context, PersonalMessageActivity::class.java).apply {
                this.putExtra("receiverID", ambassador.userId)
        ...
        btnViewProfile.setOnClickListener {
            Intent(context, UserProfileActivityKt::class.java).apply {
                this.putExtra("uID", ambassador.userId)
                context.startActivity(this)
                epicDialog.dismiss()
            }
        }
