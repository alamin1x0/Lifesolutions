...
class AllCampusAmbassadorView : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_all_campus_ambassador_view)
    ...
    private fun getAmbassadorsLists() {
        ...
        val ref = FirebaseDatabase.getInstance().getReference("CampusAmbassadors").orderByKey().limitToLast(20)
    ...
    private fun initAmbassadors(users: List<Ambassador>) {
        if (users.isNotEmpty()) {
            rv_all_ambassador_list.apply {
                ...
                adapter = AllAmbassadorViewAdapter(this@AllCampusAmbassadorView, users)
            }
        }
    }
}